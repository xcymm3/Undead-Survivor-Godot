...
func _ready() -> void:
	...
	block("SouthGround",Vector3(0,-.55,103.9),Vector3(160,1,272.2),"72775a",true,false)
	block("NorthGround",Vector3(0,-.55,-143.9),Vector3(160,1,192.2),"72775a",true,false)
	for route in [Layout.ROUTE,Layout.SHOP_ROUTE,Layout.STREET_PANEL_ROUTE,Layout.SHOP_RETURN,Layout.PUMP_ROUTE,Layout.PUMP_INTERIOR,Layout.VALVE_ROUTE,Layout.FINISH_ROUTE,Layout.LOADING_ROUTE,Layout.REPAIR_ROUTE,Layout.SERVICE_ROUTE]:
	...
	for rect in Layout.PUMP_EQUIPMENT:
		block("PumpEquipment",Vector3(rect.get_center().x,1.5,rect.get_center().y),Vector3(rect.size.x,3,rect.size.y),"587b79")
	for i in range(Layout.PUMP_INTERIOR.size()-1):
		var a: Vector2 = Layout.PUMP_INTERIOR[i]
		var b: Vector2 = Layout.PUMP_INTERIOR[i+1]
		var stripe = block("InspectionLine",Vector3((a.x+b.x)/2,.015,(a.y+b.y)/2),Vector3(.18,.025,a.distance_to(b)),"e3c168",false)
		stripe.rotation.y = atan2(b.x-a.x,b.y-a.y)
	for point in [Layout.SHOP,Layout.STREET_PANEL,Layout.LEAK,Layout.PUMP,Layout.VALVE]:
	...
	for info in [["E 领取检修钥匙",Layout.SHOP],["E 解锁检修通道",Layout.STREET_PANEL],["1 · E 关闭隔离阀",Layout.LEAK],["2 · E 复位断路器",Layout.PUMP],["E 恢复门供电",Layout.VALVE]]:
		sign_at(info[0],Vector3(info[1].x,2.3,info[1].y-1),3.5)
	for info in [["商铺检修入口 ←",Vector2(-35,80)],["西侧泵房 ←",Vector2(8,-99)],["东侧阀站 →",Vector2(35,-77)],["商铺入口 ↑",Vector2(-55,87)],["检修岗亭 →",Vector2(18,75)],["检修通道 ←",Vector2(48,78)],["泵房入口 ↑",Vector2(-52,-94)],["绕设备右侧 ↑",Vector2(-47,-105)],["断路器 ←",Vector2(-47,-116)],["供电后沿路返回安全屋 ←",Vector2(70,-126)]]:
	...
	for side in [-1,1]: block("Boundary",Vector3(side*79,3,0),Vector3(2,6,480),"425747")
	for z in [-239,239]: block("Boundary",Vector3(0,3,z),Vector3(160,6,2),"425747")
	...
	for item in Layout.ITEMS:
		supplies[item.id] = block(item.id,Vector3(item.pos.x,.3,item.pos.y),Vector3(1.8,.15,2.2) if item.kind == "ammo" else Vector3(.7,.9,.6),"b56c4a" if item.kind == "med" else "b6ab73",false)
		supply_labels[item.id] = sign_at("医疗 +" if item.kind == "med" else "随机枪械 / 手雷",Vector3(item.pos.x,2.5 if item.kind == "ammo" else 1.6,item.pos.y),3.4 if item.kind == "ammo" else 1.8)
	...
	for scene in ["loading","repair","service"]: add_child(load("res://scenes/campaign_"+scene+".tscn").instantiate())
	for route in [Layout.LOADING_ROUTE,Layout.REPAIR_ROUTE,Layout.SERVICE_ROUTE]:
		for i in range(route.size()-1):
			var a: Vector2 = route[i]
			var b: Vector2 = route[i+1]
			var stripe = block("ServiceGuide",Vector3((a.x+b.x)/2,.015,(a.y+b.y)/2),Vector3(.16,.025,a.distance_to(b)),"dfbd64",false)
			stripe.rotation.y = atan2(b.x-a.x,b.y-a.y)
...
func sync(state: Dictionary) -> void:
	for objective in preload("res://scripts/campaign_objectives.gd").NODES:
		if objective_labels.has(objective.id): objective_labels[objective.id].text = ("已完成 · " if state.get(objective.id,false) else "E · ")+objective.label
	for loot in state.get("loot",[]):
		if not loot_views.has(loot.id):
			...
			add_child(model)
			model.position = Vector3(loot.pos.x,.60,loot.pos.y)
			model.scale = Vector3.ONE*.35
