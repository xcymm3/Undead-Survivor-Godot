...
try {
  ...
  const report = { status: failure ? 'failed' : 'passed', startedAt, finishedAt: new Date().toISOString(),
    ...
    boundaries: ['覆盖生存双地图与灰松渡口战役；包含内部输入单人/多人整关遍历和真实 ENet 双人/四人整关合作。', '三张地图分别运行本机 ENet 双人和四人检查；不代表 Steam 双账号验证。真人 8～12 分钟节奏与原生 GPU 质量待验收。', '网页使用独立无界面 Chromium、实际键鼠输入及只读遥测；截图不能证明主观手感或原版地图视觉一致性。', release ? '本次包含 Windows 导出、无窗口 EXE 冒烟和 ZIP 打包。' : '本次为 npm run verify 全量验收，不包含 verify:release 的 Windows EXE 导出与打包。'] };
