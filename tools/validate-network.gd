...
func _process(_dt: float) -> bool:
	...
	if started > 0 and game.sim:
		...
		if not host and age > .6 and not sent_jump:
			...
			sent_jump = true
		if not host:
			# Keep requesting crouch until an authoritative snapshot confirms it.
			# Fixed wall-clock windows lose the entire action under a busy host.
			if age > 2.2 and not remote_crouched: Input.action_press("crouch")
		...
		if (remote_stood and age > (5.0 if host else 4.5)) or age > 12.0:
