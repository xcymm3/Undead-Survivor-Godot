...
if ($LASTEXITCODE -ne 0) { throw 'Version metadata mismatch.' }
...
$buildRoot = Join-Path $projectRoot 'build/payload'
New-Item -ItemType Directory -Force -Path $buildRoot | Out-Null
& $engine --headless --path $projectRoot --editor --import --quit
if ($LASTEXITCODE -ne 0) { throw 'Import failed.' }
& $engine --headless --path $projectRoot --export-release 'Windows Steam x64' (Join-Path $buildRoot 'Undead-Survivor-Godot.exe')
...
Copy-Item -LiteralPath (Join-Path $projectRoot 'VERSION') -Destination $buildRoot -Force
...
& (Join-Path $PSScriptRoot 'build-single-exe.ps1')
