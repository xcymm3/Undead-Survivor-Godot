...
func stage_name(state: Dictionary) -> String:
	if not state.departed: return "departure"
	if not state.shop_key: return "street_shop"
	if not state.loading_power: return "loading_yard"
	if not state.shop_open: return "checkpoint"
	if not state.milestones.has("bridge_start"): return "yard_drain"
	if not state.gate_open: return "bridge"
	if not state.leak_closed: return "pump_entry"
	if not state.pump_fault_b: return "repair_workstations"
	if not state.pump_ready: return "pump_breaker"
	if not state.power_ready: return "valve"
	if not state.exit_control: return "service_corridor"
	return "escape"
...
func calibrated(profile: String, seed_value: int) -> Dictionary:
	...
	for i in 24000:
		...
		var stage: String = stage_name(state)
		if stage != last_stage:
	...
	return {"profile":profile,"seed":seed_value,"won":game.sim.won,"failed":game.sim.failed,"timeout":not game.sim.won and not game.sim.failed,"seconds":game.sim.elapsed,"hp":p.hp,"kills":game.sim.kills,"shots":p.shots,"ammo":p.ammo[p.primary],"reserve":p.reserve,"task":driver.index,"stages":stages,"diagnostics":game.sim.campaign.diagnostics(),"turn_degrees":rad_to_deg(driver.turn_rate),"aim_error_degrees":rad_to_deg(driver.error_angle),"reaction_seconds":driver.reaction,"observation_seconds":driver.observation}
func run() -> void:
	...
	f.store_string(JSON.stringify({"runs":results,"input_violations":failures,"boundary":"Synthetic solo inputs with known route and enemy telemetry; not human 8-15 minute acceptance. Failure is retained, never rerolled."},"  "))
	f.close()
	game.return_home()
	game.queue_free()
	await process_frame
