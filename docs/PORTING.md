...
- 河床网格、物理碰撞与僵尸高度共享 `Data` 的河道剖面。CPU 命中姿态和 GPU 批量渲染共用敌人根变换，河中跳跃闪避按玩家与僵尸的相对高度判断。无窗口验证包含两岸步行出入、床面/缓坡碰撞采样、步行/跳跃与追击距离对照，以及真实浏览器输入涉水、起跳和过河。
...
地图烘焙、HUD 控件、空间音频、显示快照插值和 InputMap 调整见 [NATIVE_COMPONENTS.md](NATIVE_COMPONENTS.md)。全量流程现已纳入原生组件和地图检查，以及两张地图的 ENet 双人/四人验证。当前验收结果以 artifacts/acceptance.md 和源码摘要门禁为准，历史报告不代表当前源码。
