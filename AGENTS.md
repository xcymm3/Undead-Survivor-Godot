...
- `npm run verify:release` 自动先执行全量技术测试，再完成导出、原生 EXE 无窗口冒烟和 ZIP 打包；水晶防线单人自动试玩保留执行并在报告的“玩法观察”中记录胜负、波次、血量与命中结果，但其胜负不阻塞 EXE 打包或发布。页面异常、路线越界、输入安全和其他浏览器技术检查仍是硬门禁。正式分发使用 Actions 全绿后的 artifact，不能把导出成功当作游戏验收成功。
