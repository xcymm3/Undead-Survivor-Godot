...
test('crawler stays prone in movement attack and death software views', async ({page}, info) => {
  const errors=[];
  ...
  for(const [name,extra] of [['idle',{}],['left',{crawler_moving:true,crawler_gait:Math.PI/2}],['right',{crawler_moving:true,crawler_gait:Math.PI*1.5}],['attack',{crawler_attack:.5}],['side',{crawler_moving:true,crawler_gait:Math.PI/2,crawler_heading:Math.PI/2}],['death-start',{crawler_down:.85}],['death-mid',{crawler_down:.55}],['death-final',{crawler_down:.25}],['death-side',{crawler_down:.25,crawler_heading:Math.PI/2}]]) {
