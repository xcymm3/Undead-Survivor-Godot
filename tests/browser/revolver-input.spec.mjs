...
test('real input fires cancels and completes revolver reload', async ({ page }, info) => {
  ...
  expect(await page.evaluate(() => window.__survivorSnapshot.map_id)).toBe('outpost');
  ...
  await page.waitForFunction(() => window.__survivorSnapshot?.mode === 'survival');
  ...
  await page.waitForFunction(() => !window.__survivorSnapshot.player.reloading && window.__survivorSnapshot.player.weapon === 0);
